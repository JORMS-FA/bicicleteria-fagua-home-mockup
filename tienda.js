...
const shopProducts=[
  {id:611,name:'Bicicleta GW Lynx 3×7 Velocidades MTB Rin 29',category:'MTB',price:1250000,image:shopAssets+'product-611.png',url:'/producto/bicicleta-gw-lynx-3x7-velocidades-aluminio-mtb-rin-29/',stock:true,tag:'MÁS ELEGIDA'},
...
menuToggle.addEventListener('click',()=>setMenu(!nav.classList.contains('is-open')));navScrim.addEventListener('click',()=>setMenu(false));navClose.addEventListener('click',()=>setMenu(false));nav.querySelectorAll('.site-nav__link').forEach(link=>link.addEventListener('click',()=>setMenu(false)));document.addEventListener('keydown',event=>{if(event.key==='Escape'&&nav.classList.contains('is-open'))setMenu(false)});window.addEventListener('resize',syncMenuForViewport);syncMenuForViewport();
